...
async def permanently_delete_dataset(
    ...
    # Dataset Default-View Snapshot (Visualize Performance plan, Phase 1):
    # snapshots/{dataset_id}/snapshot.json lives outside the per-file loop
    # above — it's generated by a separate Celery task keyed by dataset_id
    # directly, not tied to any one DatasetFile. Deleted unconditionally
    # (delete() is idempotent for a missing key on S3-compatible stores)
    # since not every dataset has had one generated yet.
    snapshot_storage = get_storage_backend("vps_minio")
    snapshot_storage.delete(default_bucket_for("vps_minio"), snapshot_key(dataset.id))
