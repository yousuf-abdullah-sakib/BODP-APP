...
def apply_scope_mask(df: pd.DataFrame, scope: dict) -> pd.DataFrame:
    ...
    exact-match on parameter/source columns wherever both the scope field
    and a matching column are present. Any scope field with no matching
    column in this particular file is silently skipped rather than
    erroring — not every dataset's file has every column (e.g. a
    single-parameter dataset has no "parameter" column to filter on)."""
    ...
    for scope_field, aliases in (("parameter", ("parameter", "variable")), ("source", ("source",))):
