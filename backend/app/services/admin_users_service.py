...
async def create_user_via_invite(
    ...
    email_service.send_admin_invite_email(user.email, user.full_name, token, is_admin=False)
...
async def set_user_status(
    ...
    verb = "Suspended" if status == UserStatus.SUSPENDED.value else "Activated"
    await write_audit_log(
        db,
        actor=actor,
        action=f"{verb} user",
